...
func TestNewAgentInstance_BlocksWorkspaceMemoryFilesInMuninnMode(t *testing.T) {
	...
	cfg := &config.Config{
		...
		Tools:  config.ToolsConfig{ReadFile: config.ReadFileToolConfig{Enabled: true}, Exec: config.ExecConfig{ToolConfig: config.ToolConfig{Enabled: true}}},
		Memory: config.MemoryConfig{Provider: config.MemoryProviderMuninnDB, MuninnDB: &config.MuninnDBConfig{MCPEndpoint: "http://127.0.0.1:8750/mcp", Vault: "default"}},
	}
	provider := &mockProvider{}
	agent := NewAgentInstance(nil, &cfg.Agents.Defaults, cfg, provider)
	...
	execResult := agent.Tools.ExecuteWithContext(context.Background(), "exec", map[string]any{"command": "Get-Content .\\memory\\MEMORY.md"}, "cli", "direct", "", nil)
	if !execResult.IsError {
