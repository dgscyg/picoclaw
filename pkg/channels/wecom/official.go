...
const (
	...
	wecomOfficialCmdUploadMediaInit   = "aibot_upload_media_init"
	wecomOfficialCmdUploadMediaChunk  = "aibot_upload_media_chunk"
	wecomOfficialCmdUploadMediaFinish = "aibot_upload_media_finish"
	...
	wecomOfficialUploadChunkSize      = 512 * 1024
	wecomOfficialUploadMaxChunks      = 100
	wecomOfficialImageMaxBytes        = 10 * 1024 * 1024
	wecomOfficialVideoMaxBytes        = 10 * 1024 * 1024
	wecomOfficialVoiceMaxBytes        = 2 * 1024 * 1024
	wecomOfficialFileMaxBytes         = 20 * 1024 * 1024
)
...
type wecomOfficialPreparedMedia struct {
	UploadType  string
	Filename    string
	ContentType string
	Data        []byte
...
// SendFinal implements channels.FinalMessageCapable.
// It allows wecom_official to finish callback-scoped reply streams immediately
// without leaking channel-specific finality fields into the shared bus model.
func (c *WeComOfficialChannel) SendFinal(ctx context.Context, msg bus.OutboundMessage) error {
	if !c.IsRunning() {
		return channels.ErrNotRunning
	}
	if strings.TrimSpace(msg.ChatID) == "" {
		return fmt.Errorf("wecom_official send final: empty chat id: %w", channels.ErrSendFailed)
	}
	task := c.activeReplyTask(msg.ChatID, msg.ReplyTo)
	if task != nil {
		return c.sendFinalReplyMarkdown(ctx, task, msg.ChatID, msg.Content)
	}
	return c.sendMarkdownMessage(ctx, msg.ChatID, msg.Content)
}

func (c *WeComOfficialChannel) SendMedia(ctx context.Context, msg bus.OutboundMediaMessage) error {
	if !c.IsRunning() {
		return channels.ErrNotRunning
	}
	if strings.TrimSpace(msg.ChatID) == "" {
		return fmt.Errorf("wecom_official send media: empty chat id: %w", channels.ErrSendFailed)
	}
	if len(msg.Parts) == 0 {
		return nil
	}
	...
	if store == nil {
		return fmt.Errorf("wecom_official send media: no media store: %w", channels.ErrSendFailed)
	}
	...
	for idx, part := range msg.Parts {
		...
		if err != nil {
			return fmt.Errorf("wecom_official send media resolve %s: %w", part.Ref, channels.ErrSendFailed)
		}

		prepared, err := prepareWeComOfficialOutboundMedia(localPath, meta, part)
		if err != nil {
			return err
		}
		if prepared.Note != "" {
			logger.WarnCF("wecom_official", "Downgrading outbound media type", map[string]any{
				"chat_id":      msg.ChatID,
				"ref":          part.Ref,
				"filename":     prepared.Filename,
				"content_type": prepared.ContentType,
				"upload_type":  prepared.UploadType,
				"note":         prepared.Note,
			})
		}

		mediaID, err := c.uploadWeComMedia(ctx, prepared)
		if err != nil {
			return err
		}

		useReply := task != nil && !replyConsumed && idx == 0
		if useReply {
			if err := c.sendReplyMedia(ctx, task, prepared.UploadType, mediaID); err != nil {
				return err
			}
			replyConsumed = true
			task = nil
		...
		if err := c.sendMediaMessage(ctx, msg.ChatID, prepared.UploadType, mediaID); err != nil {
			return err
		}
	}
...
func buildWeComOfficialMessageSessionKey(chatID string, msg wecomOfficialMessage, reqID string) string {
	if anchor := strings.TrimSpace(msg.MsgID); anchor != "" {
		return buildWeComOfficialConversationKey(chatID, "msg", anchor)
	}
	return buildWeComOfficialConversationKey(chatID, "req", reqID)
}

func buildWeComOfficialEventSessionKey(chatID, taskID, reqID string) string {
	if anchor := strings.TrimSpace(taskID); anchor != "" {
		return buildWeComOfficialConversationKey(chatID, "task", anchor)
	}
	return buildWeComOfficialConversationKey(chatID, "req", reqID)
}

func buildWeComOfficialConversationKey(chatID, anchorKind, anchor string) string {
	...
	return fmt.Sprintf("wecom_official:%s:%s:%s", chatID, anchorKind, anchor)
}

func prepareWeComOfficialOutboundMedia(
	localPath string,
	meta media.MediaMeta,
	part bus.MediaPart,
) (wecomOfficialPreparedMedia, error) {
	data, err := os.ReadFile(localPath)
	if err != nil {
		return wecomOfficialPreparedMedia{}, fmt.Errorf(
			"wecom_official read outbound media %s: %w",
			localPath,
	...
	if len(data) == 0 {
		return wecomOfficialPreparedMedia{}, fmt.Errorf(
			"wecom_official outbound media is empty: %w",
			channels.ErrSendFailed,
		)
	}

	contentType := firstNonEmptyString(part.ContentType, meta.ContentType)
	if strings.TrimSpace(contentType) == "" {
		detectedType, _ := detectWeComOfficialMediaType(data, strings.TrimSpace(part.Type))
		contentType = detectedType
	}

	filename := firstNonEmptyString(part.Filename, meta.Filename, filepath.Base(localPath))
	if strings.TrimSpace(filename) == "" {
		filename = fmt.Sprintf("picoclaw-media-%d", time.Now().UnixNano())
	}

	uploadType := inferWeComOfficialUploadType(strings.TrimSpace(part.Type), contentType, filename, data)
	finalType, note, err := applyWeComOfficialOutboundMediaLimits(data, uploadType, contentType, filename)
	if err != nil {
		return wecomOfficialPreparedMedia{}, err
	}
	filename = ensureWeComOfficialFilename(filename, contentType, finalType, data)

	return wecomOfficialPreparedMedia{
		UploadType:  finalType,
		Filename:    filename,
...
func inferWeComOfficialUploadType(partType, contentType, filename string, data []byte) string {
	...
	case ".mp4", ".mov", ".avi", ".webm", ".mkv":
		return "video"
	case ".amr", ".mp3", ".wav", ".ogg", ".m4a", ".aac", ".opus":
		return "voice"
	}
...
func applyWeComOfficialOutboundMediaLimits(
	data []byte,
	uploadType, contentType, filename string,
) (string, string, error) {
	size := len(data)
	if size > wecomOfficialFileMaxBytes {
		return uploadType, "", fmt.Errorf(
			"wecom_official media %q exceeds 20MB limit: %w",
			filename,
			channels.ErrSendFailed,
		)
	}
	...
	switch uploadType {
	case "image":
		if size > wecomOfficialImageMaxBytes {
			return "file", fmt.Sprintf("image %.2fMB exceeds 10MB, sending as file", fileSizeMB), nil
		}
	case "video":
		if size > wecomOfficialVideoMaxBytes {
			return "file", fmt.Sprintf("video %.2fMB exceeds 10MB, sending as file", fileSizeMB), nil
		}
	case "voice":
		if !isWeComOfficialAMR(contentType, filename, data) {
			return "file", fmt.Sprintf("voice format %q is not AMR, sending as file", contentType), nil
		}
		if size > wecomOfficialVoiceMaxBytes {
			return "file", fmt.Sprintf("voice %.2fMB exceeds 2MB, sending as file", fileSizeMB), nil
		}
	}
...
func isWeComOfficialAMR(contentType, filename string, data []byte) bool {
	if strings.EqualFold(strings.TrimSpace(contentType), "audio/amr") {
		return true
	}
	if strings.EqualFold(filepath.Ext(filename), ".amr") {
		return true
	}
	return bytes.HasPrefix(data, []byte("#!AMR"))
}

func ensureWeComOfficialFilename(filename, contentType, uploadType string, data []byte) string {
	filename = strings.TrimSpace(filename)
	if filename == "" {
		filename = "picoclaw-media"
	}
	...
	if ext == "" {
		_, detectedExt := detectWeComOfficialMediaType(data, uploadType)
		ext = detectedExt
	}
	if ext == "" {
		switch uploadType {
...
func (c *WeComOfficialChannel) uploadWeComMedia(
	ctx context.Context,
	media wecomOfficialPreparedMedia,
) (string, error) {
	totalChunks := (len(media.Data) + wecomOfficialUploadChunkSize - 1) / wecomOfficialUploadChunkSize
	if totalChunks == 0 {
		return "", fmt.Errorf("wecom_official upload media: no data: %w", channels.ErrSendFailed)
	}
	if totalChunks > wecomOfficialUploadMaxChunks {
		return "", fmt.Errorf(
			"wecom_official upload media: %d chunks exceed limit %d: %w",
			totalChunks,
			wecomOfficialUploadMaxChunks,
			channels.ErrSendFailed,
		)
	}

	sum := md5.Sum(media.Data)
	initFrame, err := c.sendCommandAndWait(ctx, wecomOfficialCmdUploadMediaInit, map[string]any{
		"type":         media.UploadType,
		"filename":     media.Filename,
		"total_size":   len(media.Data),
	...
	if err := json.Unmarshal(initFrame.Body, &initResult); err != nil {
		return "", fmt.Errorf("wecom_official upload init decode failed: %w", err)
	}
	if strings.TrimSpace(initResult.UploadID) == "" {
		return "", fmt.Errorf("wecom_official upload init returned empty upload_id: %w", channels.ErrSendFailed)
	}

	for chunkIndex := 0; chunkIndex < totalChunks; chunkIndex++ {
		start := chunkIndex * wecomOfficialUploadChunkSize
		end := min(start+wecomOfficialUploadChunkSize, len(media.Data))
		chunkData := media.Data[start:end]
		_, err := c.sendCommandAndWait(ctx, wecomOfficialCmdUploadMediaChunk, map[string]any{
			"upload_id":   initResult.UploadID,
			"chunk_index": chunkIndex,
			"base64_data": base64.StdEncoding.EncodeToString(chunkData),
		}, wecomOfficialSendTimeout)
		if err != nil {
			return "", err
		}
	...
	finishFrame, err := c.sendCommandAndWait(ctx, wecomOfficialCmdUploadMediaFinish, map[string]any{
		"upload_id": initResult.UploadID,
	}, wecomOfficialSendTimeout)
	if err != nil {
		return "", err
	...
	if err := json.Unmarshal(finishFrame.Body, &finishResult); err != nil {
		return "", fmt.Errorf("wecom_official upload finish decode failed: %w", err)
	}
	if strings.TrimSpace(finishResult.MediaID) == "" {
		return "", fmt.Errorf("wecom_official upload finish returned empty media_id: %w", channels.ErrSendFailed)
	}
	return finishResult.MediaID, nil
}

func (c *WeComOfficialChannel) sendReplyMedia(
	ctx context.Context,
	task *wecomOfficialReplyTask,
	mediaType, mediaID string,
) error {
	if task == nil {
		return fmt.Errorf("missing reply task for media reply: %w", channels.ErrSendFailed)
	}
	body := map[string]any{
		"msgtype": mediaType,
	...
	_, err := c.sendCommandWithReqIDAndWait(ctx, task.ReqID, wecomOfficialCmdRespondMessage, body, wecomOfficialSendTimeout)
	if err != nil {
		return err
	}
	return nil
...
func (c *WeComOfficialChannel) sendMediaMessage(
	ctx context.Context,
	chatID, mediaType, mediaID string,
) error {
	_, err := c.sendCommandAndWait(ctx, wecomOfficialCmdSendMessage, map[string]any{
		"chatid":  chatID,
		"msgtype": mediaType,
...
func (c *WeComOfficialChannel) processIncomingMessage(frame wecomOfficialFrame, msg wecomOfficialMessage) {
	...
	sessionKey := buildWeComOfficialMessageSessionKey(chatID, msg, frame.Headers.ReqID)
	c.HandleMessageWithSessionKey(c.ctx, peer, msg.MsgID, userID, chatID, content, mediaRefs, metadata, sessionKey, sender)
}
...
func (c *WeComOfficialChannel) handleTemplateCardEvent(
	...
	sessionKey := buildWeComOfficialEventSessionKey(chatID, taskID, frame.Headers.ReqID)
	c.HandleMessageWithSessionKey(c.ctx, peer, msg.MsgID, userID, chatID, content, nil, metadata, sessionKey, sender)
}
...
func (c *WeComOfficialChannel) sendFinalReplyMarkdown(
	ctx context.Context,
	task *wecomOfficialReplyTask,
	chatID, content string,
) error {
	...
	return c.sendFinalReplyChunk(ctx, task, content)
}
...
func (c *WeComOfficialChannel) sendFinalReplyChunk(
	ctx context.Context,
	task *wecomOfficialReplyTask,
	content string,
) error {
	...
	var err error
	if sendWithCard {
		err = c.sendReplyStreamWithCard(ctx, task.ReqID, task.StreamID, accumulated, true, true)
		if err == nil {
			task.cardSent = true
		}
	} else {
