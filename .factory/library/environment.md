...
- Current local lint reality: the installed `golangci-lint` 2.4.0 binary rejects the disabled `modernize` entry in `.golangci.yaml`, and repo-wide `golangci-lint run` also reports many legacy non-Muninn findings. For single-feature validation, prefer change-scoped lint (for example `--new-from-rev <feature-base>`) and report the branch-wide lint debt separately.
