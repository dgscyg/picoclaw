...
- `pkg/channels/weixin/weixin.go` (`WeixinChannel`): Tencent iLink REST channel. It polls upstream message updates, persists the sync buffer for resume, tracks per-user `context_token` for reply continuity, and exposes explicit `channels.weixin.{token,base_url,cdn_base_url,proxy,allow_from,reasoning_channel_id}` config instead of the older root-level sample layout.
