...
- `pkg/bus/types.go` (`InboundMessage`, `OutboundMessage`, `OutboundMediaMessage`): Structured message types for bus communication with routing metadata.
...
- **1. Receive:** Channel receives platform message and constructs `InboundMessage` with `Peer` and `SenderInfo`.
- **2. Publish:** Channel calls `bus.PublishInbound(ctx, msg)` via `BaseChannel.HandleMessage` (`pkg/channels/base.go`).
...
- **1. Generate:** Session key format is "channel:chatID" (e.g., "telegram:123456").
- **2. Prefix:** Agent-specific sessions use "agent:" prefix for isolation.
- **3. Sanitize:** File persistence replaces ":" with "_" for Windows compatibility (`pkg/session/manager.go:153-155`).
...
- **1. Publish:** Agent publishes `OutboundMessage` via `bus.PublishOutbound(ctx, msg)`.
- **2. Subscribe:** Channel manager receives via `bus.SubscribeOutbound(ctx)` (`pkg/channels/manager.go`).
...
- **5. Send:** `sendWithRetry` with exponential backoff on temporary errors.
