...
- **Context:** Uses `ToolChannel/ToolChatID` from context for routing; on `wecom_official`, `separate_message=true` sends proactive markdown, not template cards. When `message` successfully sends to the current conversation, it marks the turn as consumed so the agent does not append a second assistant reply. Cross-target sends and `separate_message=true` do not consume the current turn. In cron-scoped agent turns, `message` is the preferred way to notify users; once it sends, trailing LLM narration is suppressed.
...
- **Parameters:** `path` (string), `filename` (string, optional)
- **Returns:** Media reference for channel delivery through the bus `OutboundMediaMessage` pipeline
- **MIME Detection:** Uses magic bytes for type detection
- **Context:** Uses `ToolChannel/ToolChatID` to decide the target conversation. On `wecom_official`, the first attachment can reuse the current callback reply token when `reply_to` is still live; otherwise the channel uploads media over the official websocket `aibot_upload_media_init/chunk/finish` flow and sends it proactively.
- **Channel Limits:** `wecom_official` accepts images/videos up to 10MB, AMR voice up to 2MB, and files up to 20MB. Unsupported or oversized image/video/voice uploads degrade to `file` when still within the 20MB hard limit.
...
- **Source:** `pkg/tools/search_tool.go`
- **Parameters:** `query` (string, required)
- **Returns:** Matching hidden tools and temporarily promotes them into the visible tool set
- **Use Case:** Natural-language discovery for hidden MCP tools and other non-core tools
- **Recall Notes:** The BM25 corpus includes tool name, description, and a normalized variant that splits `snake_case` / punctuation-heavy identifiers into plain terms, so queries like `control air conditioner device` can match MCP names such as `mcp_armand_dl645t_control_ac_device`

### tool_search_tool_regex
- **Source:** `pkg/tools/search_tool.go`
- **Parameters:** `pattern` (string, required)
- **Returns:** Matching hidden tools and temporarily promotes them into the visible tool set
- **Use Case:** Precise fallback when tool names are highly structured or when the caller already knows part of the identifier
- **Guardrails:** Empty patterns are rejected and regex length is capped
