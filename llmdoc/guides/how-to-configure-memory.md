...
本指南介绍如何在 PicoClaw 中配置记忆后端，以及如何在文件存储和 MuninnDB 之间切换。对于 MuninnDB，当前配置同时覆盖透明记忆层的 REST 通道和手动 MCP 工具通道。
...
| 字段 | 含义 |
|------|------|
| `mcp_endpoint` | Muninn MCP 服务地址，供 `mcp_muninn` 等手动工具使用 |
| `rest_endpoint` | Muninn REST 服务地址，供透明 recall/capture 与兼容记忆后端使用 |
| `vault` | 记忆仓名称 |
| `rest_api_key` | REST Bearer Token |
...
- 当 `provider = "muninndb"` 时，`mcp_endpoint`、`rest_api_key`、`mcp_api_key` 必填
- `vault` 不能为空，默认值为 `default`

说明：

- `rest_endpoint` 可单独配置到 REST 端口；若省略，会从 `mcp_endpoint` 推导兼容 REST 地址。
- 旧版 `memory.muninndb.api_key` 已不再支持，必须拆分为 `rest_api_key` 和 `mcp_api_key`。
...
```bash
export MUNINNDB_MCP_ENDPOINT=http://127.0.0.1:8750/mcp
export MUNINNDB_REST_ENDPOINT=http://127.0.0.1:8475
export MUNINNDB_VAULT=default
export MUNINNDB_REST_API_KEY=secret-rest
export MUNINNDB_MCP_API_KEY=secret-mcp
...
同时，当 `provider = "muninndb"` 时：

- 透明 recall / durable capture 走 REST 通道。
- 手动 Muninn MCP 工具走 MCP 通道。
- durable capture 不只覆盖英文正向偏好，也覆盖更广泛的 durable signal，例如负向偏好 `我不喜欢出差`、操作约束和项目决策。
...
现象：启动时报错 `memory.muninndb.mcp_endpoint is required`

处理方法：

- 检查 `memory.provider` 是否设为 `muninndb`
- 确认 `memory.muninndb.mcp_endpoint` 已配置
- 如果使用环境变量，确认变量已正确导出
...
- 检查 MuninnDB REST/MCP 服务地址、端口和对应 API Key
...
- MuninnDB 后端检查 REST `activate` 接口是否能返回 engram
